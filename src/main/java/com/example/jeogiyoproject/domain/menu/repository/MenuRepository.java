...
public interface MenuRepository extends JpaRepository<Menu,Long> {
    @Query("SELECT DISTINCT new com.example.jeogiyoproject.domain.menu.dto.category.response.MenuCategoryListResponseDto(c.id, c.name) " +
    ...
    @Query("SELECT DISTINCT new com.example.jeogiyoproject.domain.menu.dto.category.response.MenuCategoryListResponseDto(c.id, c.name) " +
            "FROM MenuCategory c " +
            "WHERE c.foodStore.id = :foodStoreId")
